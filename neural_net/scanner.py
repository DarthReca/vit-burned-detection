...
class Scanner:
        ...
        for prod in self.products:
            ...
            if len(files[prod]) > 1:
                ...
                if self.df is None:
                    ...
                    activation_date = self.convert_date(
                        self.df.loc[foldername]["activation_date"]
                    )
    ...
    def get_mask(
        ...
        apply_mask_discretization: bool = True,
    ) -> np.ndarray:
        """
        ...
        if apply_mask_discretization:
            result = (
                ...
                else np.zeros((mask.shape[0], mask.shape[1], len(self.mask_intervals)))
            )
    ...
    @classmethod
    def _generate_mode_dict(
        cls, product_list: list, mode: str, pre_before: bool = True
    ) -> dict:
        """
    ...
    def get_all(
        ...
        if retrieve_mask:
            img = self.get_mask(
                ...
                apply_mask_discretization=apply_mask_discretization,
            )
            img_result.append(img)
