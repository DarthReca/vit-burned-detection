    ...
    def __init__(self, image_transfomer, combiner, name: str, pre_first: bool = False):
        ...
        if name is not None and name != "":
            ...
            first = (
                ...
                else self.image_transformer.__name__
            )
        ...
        if self.combiner is not None:
            ...
            if pre is not None and post is not None:
                result = (
                    ...
                    if self.pre_first
                    else np.concatenate([post, pre], axis=-1)
                )
...
class ImageProcessor:
    ...
    def process(
        ...
        pre_first: bool = False,
        channel_counter: list = None,
        ...
        """
        if not (
            isinstance(img_list, list)
            or (isinstance(img_list, np.ndarray) and channel_counter is not None)
        ):
            raise ValueError(
                "Invalid set of parameters. Either img_list must be list or img_list must be np.array with channel_counter specified"
            )
        ...
        out_order_len = (
            len(output_order) if "mask" not in output_order else len(output_order) - 1
        )
        if len(process_dict) != out_order_len:
            raise ValueError(
                "Invalid output_order specified: %s - Process dict keys not matching: %s"
                % (str(output_order), str(list(process_dict.keys())))
            )
        for out in output_order:
            if out not in process_dict:
                raise ValueError(
                    "Invalid product name in output_order not found in process_dict: %s"
                    % out
                )
        ...
        for idx, out in enumerate(output_order):
            ...
            post_index = (
                product_tree[out]["post"] if "post" in product_tree[out] else -1
            )
            ...
            if isinstance(img_list, list):
                ...
                pre_offset, pre_channels = self._compute_offset_channels(
                    product_tree, channel_counter, out, "pre"
                )
                post_offset, post_channels = self._compute_offset_channels(
                    product_tree, channel_counter, out, "post"
                )

                pre = self._extract_image_from_array(img_list, pre_offset, pre_channels)
                post = self._extract_image_from_array(
                    img_list, post_offset, post_channels
                )
        ...
        if "mask" not in output_order and "mask" in product_list:
            if isinstance(img_list, list):
                ...
                mask_offset, mask_channels = self._compute_offset_channels(
                    product_tree, channel_counter, "mask", ""
                )
                mask = self._extract_image_from_array(
                    img_list, mask_offset, mask_channels
                )
            result.append(mask)
    ...
    def upscale(
        self, img_list: list, product_list: list, foldername: str, concatenate: bool
    ):
        """
    ...
    def cut(
        ...
        def _cut(image, height, width, contain_mask, apply_mask_round):
            image = self._reshape_min(
                image, height, width, contain_mask, apply_mask_round
            )
            ...
            for i in range(max_i):
                for j in range(max_j):
                    vertical_slice = slice(
                        min(height * i, resolution[0] - height),
                        min(height * (i + 1), resolution[0]),
                        1,
                    )
                    horizontal_slice = slice(
                        min(width * j, resolution[1] - width),
                        min(width * (j + 1), resolution[1]),
                        1,
                    )
                    tmp = image[..., vertical_slice, horizontal_slice, :]

                    assert (
                        tmp.shape[-3:][0] == height
                        and tmp.shape[-3:][1] == width
                        and tmp.shape[-3:][2] == resolution[2]
                    )
            ...
            result = np.swapaxes(
                result, 0, -4
            )  # [n_images x] n_cuts x height x width x channels
        ...
        if isinstance(image, list):
            ...
            for img, prod in zip(image, product_list):
                ...
                tmp, cnt = _cut(
                    img, self.height, self.width, contain_mask, apply_mask_round
                )
                result.append(tmp)
