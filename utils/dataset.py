...
import numpy as np
import utils
from neural_net import ImageProcessor, Scanner
...
class SatelliteDataset(Dataset):
    ...
    def __init__(
        ...
        product_list: list,
        mode,
        filter_validity_mask,
        transform=None,
        process_dict: dict = None,
        activation_date_csv: str = None,
        folder_list: list = None,
        ignore_list: list = None,
        mask_filtering: bool = False,
        only_burnt: bool = False,
        mask_postfix: str = "mask",
        **kwargs,
    ):
        """s
        ...
        if isinstance(filter_validity_mask, bool):
        ...
        elif isinstance(filter_validity_mask, list) or isinstance(
            filter_validity_mask, set
        ):
            self.filter_validity_flag = True
            ...
            raise ValueError(
                "Invalid value for filter_validity_mask %s" % str(filter_validity_mask)
            )
        ...
        for idx, dirname in enumerate(self.folder_list):
            image, product = self.scanner.get_all(
                dirname,
                self.product_list,
                mode=mode,
                retrieve_mask=True,
                mask_postfix=self.mask_postfix,
                apply_mask_discretization=self.apply_mask_discretization,
            )
            ...
            if self.only_burnt:
                ...
                if self.mask_one_hot:
                    tmp_mask = np.argmax(tmp_mask, axis=-1, keepdims=True)
            ...
            if self.filter_validity_flag:
                for img, prod in zip(image, product):
                    ...
                    if prod == "mask":
                        continue
                    if (
                        isinstance(self.filter_validity_mask, bool)
                        and self.filter_validity_mask
                    ) or (
                        (
                            isinstance(self.filter_validity_mask, list)
                            or isinstance(self.filter_validity_mask, set)
                        )
                        and base_prod in self.filter_validity_mask
                    ):
                        bool_mask = img[:, :, -1] == 0
                        img[:, :, :-1][bool_mask] = 0
            ...
            brnt_mask = self.processor.upscale(
                [brnt_mask], ["mask"], dirname, concatenate=False
            )
            if process_dict is not None:
                image, product = self.processor.process(
                    image,
                    product,
                    process_dict,
                    return_ndarray=True,
            ...
            if self.mask_filtering:
                ...
                nonburned_mask = (
                    tmp_mask == 0 if not self.mask_one_hot else tmp_mask[..., 0] == 1
                )
            ...
            image, count = self.processor.cut(
                image,
                product,
                return_ndarray=True,
                apply_mask_round=self.apply_mask_discretization,
            )
            # image = image.astype(np.float32)
            image, mask = image[..., :-1], image[..., -1:]

            brnt_mask, _ = self.processor.cut(
                brnt_mask, ["mask"], return_ndarray=True, apply_mask_round=True
            )
    ...
    def __getitem__(self, idx):
        ...
        """
        result = {"image": self.images[idx], "mask": self.masks[idx]}
...
class AggregationDataset(SatelliteDataset):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # Aggregated channels (cloud_coverage, nvdi, gndvi, ndwi, wiw, bsi, bais2)
        cloud_coverage = np.stack(
            [
                utils.cloud_coverage(self.images[:, :, :, 2], self.images[:, :, :, 3]),
                utils.ndvi(self.images[:, :, :, 7], self.images[:, :, :, 3]),
                utils.gndvi(self.images[:, :, :, 7], self.images[:, :, :, 2]),
                utils.ndwi(self.images[:, :, :, 2], self.images[:, :, :, 7]),
                utils.wiw(self.images[:, :, :, 8], self.images[:, :, :, 11]),
                utils.bsi(
                    self.images[:, :, :, 1],
                    self.images[:, :, :, 3],
                    self.images[:, :, :, 7],
